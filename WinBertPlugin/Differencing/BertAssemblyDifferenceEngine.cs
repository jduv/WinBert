namespace WinBert.Differencing
{
    ...
    public sealed class BertAssemblyDifferenceEngine : IDifferenceEngine<Assembly, AssemblyDifferenceResult>
    {
        ...
        public BertAssemblyDifferenceEngine(IgnoreTarget[] ignoreTargets)
        {
            ...
            {
                var typeIgnoreTargets = from it in ignoreTargets where it.Type == IgnoreType.Type select it;
                this.ignoreTargets = Enumerable.ToArray<IgnoreTarget>(typeIgnoreTargets);
            }
        }
        ...
        private IList<Type> GetFilteredTypeList(Type[] types)
        {
            ...
            {
                IEnumerable<Type> availableTypes = from t in types
                                                   let ignoreTargets = 
        ...
        {
            Dictionary<string, Type> typesDictionary = new Dictionary<string, Type>();

            var filteredTypeList = this.GetFilteredTypeList(assembly.GetTypes());
            foreach (Type t in filteredTypeList)
            {
                typesDictionary.Add(t.FullName, t);
            }
